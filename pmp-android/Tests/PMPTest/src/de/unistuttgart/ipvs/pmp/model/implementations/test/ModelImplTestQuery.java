...
public class ModelImplTestQuery extends AndroidTestCase {
    ...
    protected void setUp() throws Exception {
        ...
        String sqlQueries = DatabaseSingleton.getInstance().getDatabaseHelper().readSqlFile("testsql.sql");
        DatabaseOpenHelper.executeMultipleQueries(DatabaseSingleton.getInstance().getDatabaseHelper()
    ...
    public void testAppsLength() {
        ...
        assertEquals(7, this.apps.length);
    }
    ...
    public void testAppQueryNR0() {
         ...
         * App', 0);
         */
        IApp toTestApp = this.apps[actualAppIndex];
        assertEquals("Sample#pmp.test.okapp", toTestApp.getIdentifier());
        assertEquals("OK App", toTestApp.getName());
        assertEquals("This is conformaly defined App", toTestApp.getDescription());
    ...
    /**
     * Testing the query #2, which has been modified: - no Identifier Commented, because this query didn't make it to
     * the DB
     * 
    ...
    /**
     * Testing the query #3, which has been modified: - empty Identifier Apps with empty Identifier shouldn't be in the
     * DB
     */
    ...
    /**
     * Testing the query #5, which has been modified: - no active level Commented, because this query didn't make it to
     * the DB
     * 
     * Test succeed, because broken queries shouldn't get to the DB
     */
     ...
     * 
     * Exception: NullPointerException If active level is not available, PMP should change it to the lowest available
     * level
     */
    ...
    /**
     * Testing the query #7, which has been modified: </br>- Same Identifier as in query #0, but different Name,
     * Description
     * 
     * </br></br>The first App with the same Identifier is in the DB, all the next Apps wont get in the DB with the same
     * Identifier Exception: ArrayIndexOutOfBoundException
     */
    public void testAppQueryNR7() {
        ...
        assertEquals("This is conformaly defined App 2nd", toTestApp.getDescription());
        assertEquals(0, toTestApp.getActiveServiceLevel().getLevel());
    }
    
    
    /**
     * Testing the query #0, which has been modified: </br>- empty Identifier Res with empty Identifier shouldn't be in
     * the DB
     */
    public void testResGQueryNR0() {
        /* Load query #0 */
        int actualResIndex = 0;
        IResourceGroup toTestRes = this.ress[actualResIndex];
        /*
        ...
        assertEquals("This ressource has empty identifier", toTestRes.getDescription());
    }
    ...
    public void testResGQueryNR2() {
         ...
         */
        assertEquals("Sample#pmp.test.resnodescription", toTestRes.getIdentifier());
    ...
    /**
     * Testing the query #3, which has been modified: </br>- no Identifier Commented, because this query didn't make it
     * to the DB
     * 
    ...
    /**
     * Testing the query #4, which has been modified: </br>- same Identifier as in query #1, but different Name,
     * Description
     * 
    ...
    public void testResGQueryNR4() {
        ...
        assertEquals("This ressource has same identifier resnoname", toTestRes.getDescription());
    }
    
    
    /**
     * Testing the queries for Service Level/ Privacy Level of App with the Identifier: </br>- Sample#pmp.test.okapp
     */
    
    public void testQuerySLPLokApp() {
        int actualAppIndex = 0;
        IApp app = this.apps[actualAppIndex];
    ...
    /**
     * Testing the queries for Service Level/ Privacy Level of App with the Identifier: </br>- Sample#pmp.test.noname
     */
    ...
    /**
     * Testing the queries for Service Level/ Privacy Level of App with the Identifier: </br>- ""
     */
