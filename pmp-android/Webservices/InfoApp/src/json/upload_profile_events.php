...
try {
    ...
    foreach ($rawEvents as $event) {
        ...
        if (!property_exists($event, "timestamp") || !property_exists($event, "event") ||
                !property_exists($event, "direction")) {
        ...
        $events[] = new ProfileEvent(1, $event->timestamp, $event->event,
                $event->direction, $city);

    }

    // Write events into table
    $device->getProfileEventManager()->addEvents($events);
