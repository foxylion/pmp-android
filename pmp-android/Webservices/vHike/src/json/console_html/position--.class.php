...
class position {
    ...
    private $longitude;
    /** 
     * Time stamp representing the point in time of the latest updated of the positioning data
     * @var int
     */
    private $lastUpdate;
    ...
    public static function loadPosition($user) {
        ...
        $row = $db->fetch($db->query("SELECT 
                                        ...
                                        `latitude`,
                                        `longitude`,
                                        unix_timesamp(`last_update`) AS `last_update`
     ...
     *          This may return a position with "last_update" set to "0" if there's no
    ...
    public static function loadPositionBySqlResult($result, $idFieldName = "id") {
        if (!is_array($result) || $idFieldName == null || $idFieldName == "") {
            throw new InvalidArgumentException("Result or idFieldName is invalid");
        }
        if (isset($result["latitude"]) && isset($result["longitude"]) && isset($result["last_update"])) {
	        $pos = new Position();
	        
	        $pos->id = $result[$idFieldName];
	        $pos->latitude = (float)$result["latitude"];
	        $pos->longitude = (float)$result["longitude"];
	        $pos->lastUpdate = (int)$result["last_update"];
        } else {
        	$db = Database::getInstance();
        	$db->fetch($db->query("SELECT * FROM `".DB_PREFIX."_position` WHERE user =" .$idFieldName));
        }
