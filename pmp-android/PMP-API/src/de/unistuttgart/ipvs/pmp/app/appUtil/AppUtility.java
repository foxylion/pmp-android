 ...
 * set of the app) and return a complete AppInformationSet.
 * 
 * @author marcus
 * 
 */
public class AppUtility {
     ...
     */
    public static AppInformationSet createAppInformationSet(
	...
	for (Locale l : ais.getNames().keySet()) {
	    Log.d("Name: " + ais.getNames().get(l) + " (Locale: "
		    + l.getLanguage() + ")");
	}
	for (Locale l : ais.getDescriptions().keySet()) {
	    Log.d("Description: " + ais.getDescriptions().get(l) + " (Locale: "
		    + l.getLanguage() + ")");
	}
	Log.d("----------------------");
	...
	for (Integer level : ais.getServiceLevels().keySet()) {
		...
		Log.d("Name: " + sl.getNames().get(l) + " (Locale: "
			+ l.getLanguage() + ")");
	    }
	    for (Locale l : sl.getDescriptions().keySet()) {
		Log.d("Description: " + sl.getDescriptions().get(l)
		...
		for (String privacyLevelIdentifier : rrg.getPrivacyLevelMap()
			.keySet()) {
		    Log.d("- Privacy Level Value: "
			    + rrg.getPrivacyLevelMap().get(
				    privacyLevelIdentifier) + " (Identifier: "
