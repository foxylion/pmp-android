...
public class CalendarAppActivity extends ListActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		final PMPServiceConnector connector = new PMPServiceConnector(
				...
				((CalendarApp) getApplicationContext()).getSignee());
			...
			public void connected() {
				if (!connector.isRegistered()) {
					new Handler().post(new Runnable() {
						...
						@Override
						public void run() {
							Dialog dialog = ProgressDialog.show(self, "Please wait...", "Registration is running.", true);
						}
					});
					((CalendarApp) self.getApplicationContext()).start(self
							.getApplicationContext());
				}
			}
