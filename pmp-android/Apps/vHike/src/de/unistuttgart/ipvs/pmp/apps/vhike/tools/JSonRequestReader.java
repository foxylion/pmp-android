...
/**
 * JSonRequestReader makes it possible to read requested JSON Objects easily It provides all the
 * methods to communicate with the web service outside.
 * 
...
public class JSonRequestReader {
	...
	public static String register(String username, String password,
			...
			String lastname, String tel, String description,
	...
	public static Profile getOwnProfile(String sid) {
		...
		if (suc) {
			profile =
					new Profile(id, username, email, firstname, lastname, tel,
							description, date,
							email_public, firstname_public, lastname_public,
							tel_public, rating_avg,
							rating_num);
			return profile;
		}
		return null;
	...
	public static Profile getProfile(String session_id, int id) {
		...
		if (suc) {
			profile =
					new Profile(userid, username, email, firstname, lastname,
							tel, description, date,
							email_public, firstname_public, lastname_public,
							tel_public, rating_avg,
							rating_num);
			return profile;
	...
	public static PositionObject getUserPosition(String sid, int user_id) {
		...
		listToParse.add(new ParamObject("user_id", String.valueOf(user_id),
				true));
	...
	public static String
			tripUpdateData(String sid, int trip_id, int avail_seats) {
		listToParse.clear();
		listToParse.add(new ParamObject("sid", sid, false));
	...
	public static String endTrip(String sid, int trip_id) {
		...
		if (trip_id > -1) {
			listToParse
					.add(new ParamObject("id", String.valueOf(trip_id), true));
		}
			...
			if (suc) {
				...
				if (status.equals("result")) {
					...
					for (int i = 0; i < array.size(); i++) {
						...
						QueryObject qObject =
								new QueryObject(queryid, userid, username,
										cur_lat, cur_lon,
										seats, distance);
		...
		if (object != null) {
			...
			if (suc) {
				...
				for (int i = 0; i < array.size(); i++) {
					...
					RideObject qObject =
							new RideObject(tripid, seats, cur_lat, cur_lon,
									destination,
									driverid, username, rating, distance);
	...
	public static String sendOffer(String sid, int trip_id, int query_id,
		...
		Log.d(TAG, sid);
		Log.d(TAG, String.valueOf(trip_id));
		Log.d(TAG, String.valueOf(query_id));
		Log.d(TAG, message);
			...
			if (suc) {
				...
				try {
					...
					for (int i = 0; i < array.size(); i++) {
						...
						String username = Iobject.get("username").getAsString();
						float rating = Iobject.get("rating").getAsFloat();
						float rating_num = Iobject.get("rating_num")
								.getAsFloat();
						float lat = Iobject.get("lat").getAsFloat();
						float lon = Iobject.get("lon").getAsFloat();

						OfferObject oObject =
								new OfferObject(offer_id, user_id, username,
										rating,
										rating_num, lat, lon);
	...
	public static String offer_accepted(String sid, int offer_id) {
		...
		listToParse.add(new ParamObject("offer_id", String.valueOf(offer_id),
				true));
		JsonObject object = null;
				...
				for (int i = 0; i < array_rides.size(); i++) {
					...
					if (role.equals(Constants.ROLE_DRIVER)) {
						...
						for (int j = 0; j < array_passengers.size(); j++) {
							...
							HistoryPersonObject person =
									new HistoryPersonObject(userid, username,
											rating,
											rating_num, rated);
						...
						JsonObject driver = IObject.get("driver")
								.getAsJsonObject();
					...
					Log.d(null,
							"IN READER HISTORYPERSONS" + historyPersons.size());
					HistoryRideObject ride =
							new HistoryRideObject(tripid, avail_seats,
									creation, ending,
	...
	public static String
			rateUser(String sid, int userid, int tripid, int rating) {
		listToParse.clear();
		listToParse.add(new ParamObject("sid", sid, false));
		listToParse.add(new ParamObject("recipient", String.valueOf(userid),
				true));
		listToParse.add(new ParamObject("trip", String.valueOf(tripid), true));
		listToParse
				.add(new ParamObject("rating", String.valueOf(rating), true));

		JsonObject object = null;
		try {
			object = JSonRequestProvider
					.doRequest(listToParse, "ride_rate.php");
		...
		try {
			...
			if (json != null && json.get(ERROR) == null) {
				if (json.get("trip_id") != null) {
					Trip trip = new Trip(json.get("trip_id").getAsInt(),
							Model.getInstance().getUserId(),
							json.get("avail_seats").getAsInt(),
