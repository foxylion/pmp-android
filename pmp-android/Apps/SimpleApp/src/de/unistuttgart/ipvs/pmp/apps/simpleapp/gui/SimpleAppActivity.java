...
public class SimpleAppActivity extends Activity {
    ...
    private void refreshWifi() {
    	if (!Model.getInstance().getApp().isServiceFeatureEnabled(Model.SF_WIFI_STATE)) {
    		// Wireless State SF is disabled
    		wirelessStateTextView.setText("missingSF");
    	} else {
    		wirelessStateTextView.setText((Model.getInstance().getWifi() ? "enabled" : "disabled" ));
    		wirelessToggleButton.setChecked(Model.getInstance().getWifi());
    	}
    	
    	if (!Model.getInstance().getApp().isServiceFeatureEnabled(Model.SF_WIFI_CHANGE)) {
    		// Wireless State SF is disabled
