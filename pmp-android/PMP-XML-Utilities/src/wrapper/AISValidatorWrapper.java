...
public class AISValidatorWrapper extends AISValidator {
    ...
    public List<IIssue> validateRequiredResourceGroup(IAISRequiredResourceGroup rrg, boolean attachData) {
        List<IIssue> issues = super.validateRequiredResourceGroup(rrg, attachData);
        extendAttachments(issues, attachData);
        return issues;
    }
    ...
    public List<IIssue> validateRequiredPrivacySetting(IAISRequiredPrivacySetting rps, boolean attachData) {
        List<IIssue> issues = super.validateRequiredPrivacySetting(rps, attachData);
        extendAttachments(issues, attachData);
        return issues;
    }
                ...
                for (IIssue issue : issues) {
                    ...
                    switch (type) {
                        case NAME_LOCALE_OCCURRED_TOO_OFTEN:
                            ...
                            helper.attachIBasicIS(nameLocation.getNames(), IssueType.NAME_LOCALE_OCCURRED_TOO_OFTEN,
                                    parameters);
                            
                            // Remove issue of the descrLocation
                            nameLocation.removeIssue(issue);
                        ...
                        case DESCRIPTION_LOCALE_OCCURRED_TOO_OFTEN:
                            ...
                            helper.attachIBasicIS(descrLocation.getDescriptions(),
                                    IssueType.DESCRIPTION_LOCALE_OCCURRED_TOO_OFTEN, parameters);
                            
                            // Remove issue of the descrLocation
                            descrLocation.removeIssue(issue);
                        ...
                        case SFS_CONTAIN_SAME_RRG_AND_RPS_WITH_SAME_VALUE:
                            ...
                            for (IAISServiceFeature sf : aisLocation.getServiceFeatures()) {
                                
                                // if the sf has not the issue yet
                                boolean hasIssue = false;
                                for (IIssue assignedIssue : sf.getIssues()) {
                                    if (assignedIssue.getType().equals(
                                            IssueType.SFS_CONTAIN_SAME_RRG_AND_RPS_WITH_SAME_VALUE))
                                        hasIssue = true;
                                }
                                for (String identifier : issue.getParameters()) {
                                    if (sf.getIdentifier().equals(identifier) && !hasIssue) {
                                        sf.addIssue(new Issue(IssueType.SFS_CONTAIN_SAME_RRG_AND_RPS_WITH_SAME_VALUE,
                                                sf));
                                        hasIssue = true;
                                    }
                                }
                        ...
                        case SF_IDENTIFIER_OCCURRED_TOO_OFTEN:
                            ...
                            helper.attachIIdentifierIS(identifierISs, IssueType.SF_IDENTIFIER_OCCURRED_TOO_OFTEN,
                                    parameters);
                        ...
                        case RRG_IDENTIFIER_OCCURRED_TOO_OFTEN:
                            // Cast the location (should be possible, if
                            // its this issue type)
                            IAISServiceFeature sf = (IAISServiceFeature) issue.getLocation();
                            ...
                            helper.attachIIdentifierIS(iISs, IssueType.RRG_IDENTIFIER_OCCURRED_TOO_OFTEN, parameters);
                        ...
                        case RPS_IDENTIFIER_OCCURRED_TOO_OFTEN:
                            // Cast the location (should be possible, if
                            // its this issue type)
                            IAISRequiredResourceGroup rrg = (IAISRequiredResourceGroup) issue.getLocation();
                            ...
                            helper.attachIIdentifierIS(identISs, IssueType.RPS_IDENTIFIER_OCCURRED_TOO_OFTEN,
                                    parameters);
