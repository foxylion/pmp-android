...
public class PrivacyLevels {
	...
	/**
	 * Privacy-level for reading all files stored on the SD-Card including files
	 * and directories controlled by the other privacy-levels (e.g. data in the
	 * music directory). This means, even if <code>EXTERNAL_MUSIC_READ</code> is
	 * set to false and <code>EXTERNAL_BASE_DIR_READ</code> is enabled, the
	 * access to <code>Music/</code> is allowed.
	 */
	...
	/**
	 * Privacy-level for reading all files stored in the external
	 * <code>Music/</code> directory
	 */
	...
	/**
	 * Privacy-level for reading all files stored in the external
	 * <code>Notifications/</code> directory
	 */
	...
	public PrivacyLevels(Context c) throws Exception {
		...
		String generic_warning = c.getResources().getString(
				R.string.pl_generic_warning);
		genericRead = new BooleanPrivacyLevel(generic + ": " + read, c
				.getResources().getString(R.string.pl_generic_read_desc)
				+ " "
				+ generic_warning);
		genericWrite = new BooleanPrivacyLevel(generic + ": " + write, c
				.getResources().getString(R.string.pl_generic_write_desc)
				+ " "
				+ generic_warning);
		genericList = new BooleanPrivacyLevel(generic + ": " + list, c
				.getResources().getString(R.string.pl_generic_list_desc)
				+ " "
				+ generic_warning);
		genericDelete = new BooleanPrivacyLevel(generic + ": " + delete, c
				.getResources().getString(R.string.pl_generic_delete_desc)
				+ " " + generic_warning);
		genericMakeDirs = new BooleanPrivacyLevel(generic + ": " + mkdirs, c
				.getResources().getString(R.string.pl_generic_mkdirs_desc)
				+ " " + generic_warning);

		// External base-dir access
		String extBaseDir = c.getResources().getString(
				R.string.pl_external_base_dir);
		String extBaseDirNotice = c.getResources().getString(
				R.string.pl_external_base_dir_notice);
		extBaseDirRead = new BooleanPrivacyLevel(extBaseDir + ": " + read, c
				.getResources().getString(
						R.string.pl_external_base_dir_read_desc)
				+ " " + extBaseDirNotice);
		extBaseDirWrite = new BooleanPrivacyLevel(extBaseDir + ": " + write, c
				.getResources().getString(
						R.string.pl_external_base_dir_write_desc)
				+ " " + extBaseDirNotice);
		extBaseDirList = new BooleanPrivacyLevel(extBaseDir + ": " + list, c
				.getResources().getString(
		...
		extBaseDirDelete = new BooleanPrivacyLevel(extBaseDir + ": " + delete,
				c.getResources().getString(
						...
						+ " " + extBaseDirNotice);
		extBaseDirMakeDirs = new BooleanPrivacyLevel(
				extBaseDir + ": " + delete, c.getResources().getString(
		...
		extNotificationsMakeDirs = getExternalMkdirsPl(c,
	 ...
	 *            App context
	 * @param stringTypeId
	 *            Resource id used for the type of the external directory
	 *            (music, podcasts, ...)
	 * @return Generates PL
	 */
	private BooleanPrivacyLevel getExternalReadPl(Context c, int stringTypeId) {
		String read = c.getResources().getString(R.string.read);
		return new BooleanPrivacyLevel(getResourceWithReplacedDir(c,
				R.string.pl_external, stringTypeId) + ": " + read,
				getResourceWithReplacedDir(c, R.string.pl_external_read_desc,
						stringTypeId));
	}
	...
	private BooleanPrivacyLevel getExternalWritePl(Context c, int stringTypeId) {
		String write = c.getResources().getString(R.string.write);
		return new BooleanPrivacyLevel(getResourceWithReplacedDir(c,
				R.string.pl_external, stringTypeId) + ": " + write,
				getResourceWithReplacedDir(c, R.string.pl_external_write_desc,
						stringTypeId));
	}
	...
	private BooleanPrivacyLevel getExternalListPl(Context c, int stringTypeId) {
		String list = c.getResources().getString(R.string.list);
		return new BooleanPrivacyLevel(getResourceWithReplacedDir(c,
				R.string.pl_external, stringTypeId) + ": " + list,
				getResourceWithReplacedDir(c, R.string.pl_external_list_desc,
						stringTypeId));
	}

	/**
	 * Creates a PL using strings from the resource file for deleting
	 * files/dirs.
	 * 
	 * @param c
	 *            App context
	...
	private BooleanPrivacyLevel getExternalDeletePl(Context c, int stringTypeId) {
		String delete = c.getResources().getString(R.string.delete);
		return new BooleanPrivacyLevel(getResourceWithReplacedDir(c,
				R.string.pl_external, stringTypeId) + ": " + delete,
				getResourceWithReplacedDir(c, R.string.pl_external_delete_desc,
						stringTypeId));
	}
	...
	private BooleanPrivacyLevel getExternalMkdirsPl(Context c, int stringTypeId) {
		String mkdirs = c.getResources().getString(R.string.mkdirs);
		return new BooleanPrivacyLevel(getResourceWithReplacedDir(c,
				R.string.pl_external, stringTypeId) + ": " + mkdirs,
				getResourceWithReplacedDir(c, R.string.pl_external_mkdirs_desc,
						stringTypeId));
	}

	/**
	 * Loads a source string and replaces <code>{dir}</code> with the
	 * replacement string
	 * 
	 * @param c
	 *            Application context
	 ...
	 */
	private String getResourceWithReplacedDir(Context c, int sourceId,
			int replacementId) {
	...
	public void addToResourceGroup(ResourceGroup rg) {
		...
		rg.registerPrivacyLevel(EXTERNAL_PODCASTS_MAKE_DIRS,
				extPodcastsMakeDirs);
		...
		rg.registerPrivacyLevel(EXTERNAL_RINGTONES_MAKE_DIRS,
				extRingtonesMakeDirs);
		...
		rg.registerPrivacyLevel(EXTERNAL_PICTURES_MAKE_DIRS,
				extPicturesMakeDirs);
		...
		rg.registerPrivacyLevel(EXTERNAL_DOWNLOAD_MAKE_DIRS,
				extDownloadMakeDirs);
	}
