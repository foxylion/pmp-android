...
public class PSValidator {
	...
	public PSValidator(ResourceGroup rg) {
		this.rg = rg;
	}
	...
	public void validate(String psIdentifier, String requiredValue) {
		...
		String grantedValue = this.rg.getPMPPrivacySettingValue(psIdentifier,
				EnergyConstants.APP_IDENTIFIER);
	...
	public void validate(PSBatteryTemperatureEnum reference) {
		...
		boolean failed = true;
		try {
			if (ps.permits(EnergyConstants.APP_IDENTIFIER, reference)) {
				failed = false;
			}
		} catch (PrivacySettingValueException e) {
			logSomethingWentWrong(e);
	...
	private void logSomethingWentWrong(PrivacySettingValueException e) {
		Log.e(this,
				"Something went wrong while validating the permissions for Privacy Settings.",
				e);
	}

	private void throwException(String psName, String psValueRequired) {
		throw new SecurityException(
				"The requested action requires at the PrivacySetting " + psName
						+ " with at least " + psValueRequired
						+ " as required value");
	}
