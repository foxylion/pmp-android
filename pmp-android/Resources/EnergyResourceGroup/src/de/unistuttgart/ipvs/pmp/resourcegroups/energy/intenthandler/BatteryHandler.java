...
public class BatteryHandler {
	...
	public static void handle(Intent intent, Context context) {
		...
		int healthIndicator = intent.getIntExtra(EnergyConstants.EXTRA_HEALTH,
				0);
		int statusIndicator = intent.getIntExtra(EnergyConstants.EXTRA_STATUS,
				0);
		int pluggedIndicator = intent.getIntExtra(
				EnergyConstants.EXTRA_PLUGGED, 0);
		boolean present = intent.getBooleanExtra(EnergyConstants.EXTRA_PRESENT,
				false);
		String technology = intent
				.getStringExtra(EnergyConstants.EXTRA_TECHNOLOGY);
		int temperature = intent.getIntExtra(EnergyConstants.EXTRA_TEMPERATURE,
				0);
